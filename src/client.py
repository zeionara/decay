...
HELLO_MSG = 'Welcome to the factorization program! In order to exit you need to write "' + EXIT_CODE + '" at any moment'
...
def main():
    ...
    try:
        while True:
            #get a new number for factorization; if user typed exit code instead of number - close program
            ...
            if not result:
                ...
                print(PREAMBLE, '%s = %s' % (message, ' * '.join([str(item) for item in result])))
    finally:
